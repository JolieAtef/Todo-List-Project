  ...
  <>
   ...
   <div className="bg-neutral-primary-soft block w-full p-3 md:w-8/10 mx-auto mt-3 md:p-6 border border-default rounded-base hover:shadow-2xl hover:-translate-y-1 transition-all duration-200 " key={task._id} >
          ...
          <div className="flex items-center mb-2">
           <input id="default-checkbox"  onClick={() => toggleTask(task._id)} checked={task.isCompleted}  type="checkbox" value="" className="w-5 h-5 border text-(--primary-color) focus:ring-0 border-default-medium rounded-xl bg-neutral-secondary-medium  cursor-pointer"/>
          ...
          {task.priority=="high"?<span className="bg-danger-soft border border-danger-subtle text-fg-danger-strong text-xs font-medium px-1.5 py-0.5 rounded">High</span>:<>{task.priority=="medium"? <span className="bg-(--primary-color)/10 border border-(--primary-color)/30 text-(--primary-color) text-xs font-medium px-1.5 py-0.5 rounded">medium</span>:<span className="bg-gray-100 border border-gray-subtle text-gray-500 text-xs font-medium px-1.5 py-0.5 rounded">Low</span>}</>}
          </div>
          <p className="text-(--secondary-color) mb-2">{task.description}</p>
          <div className="collection_footer flex justify-between">
           <div className={`date flex items-center gap-1.5 border px-3 rounded-full ${new Date(task.dueDate) > Date.now()?`bg-(--low-color) border-(--secondary-color)`:`border-danger-strong bg-red-100/60`}`}>
           ...
           <h6 className={`${new Date(task.dueDate) > Date.now()?`text-(--secondary-color)`:`text-danger-strong`}`}>
              {task.dueDate.split("T")[0]}
            </h6>
           </div>
          <div className="btns flex gap-2">
          <button onClick={()=>{setIsOpenMove(true) , setCurrentTask(task)}} type="button" className="inline-flex items-center justify-center  text-(--secondary-color) bg-(--low-color) hover:-translate-y-1 transition-all duration-150 hover:shadow-amber-900  shadow-xs rounded-full w-9 h-9 focus:outline-none cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
          <path strokeLinecap="round" strokeLinejoin="round" d="M5 9l-3 3m0 0l3 3m-3-3h16m3-6l-3-3m3 3l-3 3m3-3H5" />
          </svg>
           </button>

          <button onClick={()=>setIsOpenUpdate(true)} type="button" className="inline-flex items-center justify-center  text-(--secondary-color) bg-(--low-color)  hover:-translate-y-1 transition-all duration-150 hover:shadow-amber-900 shadow-xs rounded-full w-9 h-9 focus:outline-none  cursor-pointer">
          ...
          <button onClick={()=>setIsOpenDelete(true)} type="button" className="inline-flex items-center justify-center  text-(--secondary-color) bg-(--low-color)  hover:-translate-y-1 transition-all duration-150 hover:shadow-amber-900  shadow-xs rounded-full w-9 h-9 focus:outline-none cursor-pointer">
